// api/alerts.mjs — Telegram alerts for all Cronus parameters (cron-driven)
...
export default async function handler(req, res) {
  ...
  const [metrics, cover, track, traction, spend, payout, signer] = await Promise.all([
  ...
  const snap = {
    ...
    stakesResolved: num(track, ["resolved", "stats.resolved"]),
    ...
    spendRemaining: num(spend, ["remaining", "remainingToday", "today.remaining", "budgetRemaining"]),
    signerNative: num(signer, ["native", "nativeBalance", "balances.native"]),
    signerUsdc: num(signer, ["usdc", "usdcBalance", "balances.usdc"]),
  }
  ...
  for (const [name, d] of [["metrics", metrics], ["cover", cover], ["track-record", track], ["traction", traction], ["spend-limit", spend], ["agent-payout", payout]])
