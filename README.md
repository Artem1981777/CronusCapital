...
- **2026-07-02 — Market Pulse indicators in Markets section (new):** New read-only `/api/market` proxy (fail-open, always 200, no secrets) surfaces live Fear & Greed (alternative.me), BTC dominance and total market cap (coingecko) in a MARKET PULSE panel. Every value is sourced and refreshes every 2 min; on upstream failure it degrades to `n/a` / skeleton — no fabricated data (`8e2402a`).
