...
if (!s.includes(imp)) {
  if (!s.includes(anchorImp)) { console.error("нет якоря импорта"); process.exit(1) }
  s = s.replace(anchorImp, anchorImp + "\n" + imp)
}
const anchorSpread = "const ROUTES = { ...UPGRADE_ROUTES,"
if (!s.includes("...REAL_ROUTES")) {
  if (!s.includes(anchorSpread)) { console.error("нет якоря spread"); process.exit(1) }
  s = s.replace(anchorSpread, "const ROUTES = { ...UPGRADE_ROUTES, ...REAL_ROUTES,")
}
writeFileSync(P, s)
console.log("info.js: REAL_ROUTES подключены")
