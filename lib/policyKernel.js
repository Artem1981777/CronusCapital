...
// A deterministic fail-closed gate for any spending action.
// ADDITIVE: complements lib/breaker.js and lib/spendLimit.js; it replaces nothing.
// Invariant: LLM output (advice) may only NARROW a permission, never widen it.
...
export function evaluate(action = {}, ctx = {}) {
  ...
  // fail-closed: any missing or malformed limit means deny, never 'let it through'
  for (const [k, v] of [
...
// advice is an optional LLM recommendation. It can only forbid or reduce.
