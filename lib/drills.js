...
import { RUNS as FILE_RUNS } from "./drillsData.js"
...
// Runs live in the repository first: a committed file is verifiable by anyone with the
// repo, while a private cache is verifiable only by us. KV is merged in when present so
// a run recorded without a commit is still surfaced, never silently dropped.
async function readRuns() {
  const fromRepo = Array.isArray(FILE_RUNS) ? FILE_RUNS.slice() : []
  const rows = await kvCmd(["LRANGE", LOG_KEY, "0", String(KEEP - 1)])
  const fromKv = []
  if (Array.isArray(rows)) {
    for (const row of rows) {
      try { fromKv.push(typeof row === "string" ? JSON.parse(row) : row) } catch (_) {}
    }
  }
  const seen = new Set()
  const all = []
  for (const run of fromRepo.concat(fromKv)) {
    const key = String((run && run.runId) || (run && run.finishedAt) || "")
    if (key !== "" && seen.has(key)) continue
    if (key !== "") seen.add(key)
    all.push(run)
  }
  all.sort((a, b) => Date.parse((b && b.finishedAt) || 0) - Date.parse((a && a.finishedAt) || 0))
  const storage = fromRepo.length > 0 && fromKv.length > 0 ? "repo+kv" : fromRepo.length > 0 ? "repo" : fromKv.length > 0 ? "kv" : rows === null ? "repo (kv unavailable)" : "empty"
  return { runs: all.slice(0, KEEP), storage }
}
