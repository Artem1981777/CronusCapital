...
const EVM_DESTS = {
  ...
  arc: { domain: 26, chainId: 5042002, rpc: process.env.ARC_RPC || "https://rpc.testnet.arc.network", explorer: "https://testnet.arcscan.app/tx/", name: "Arc Testnet" },
}
const DOMAIN_TO_DEST = { 6: "baseSepolia", 0: "sepolia", 3: "arbitrumSepolia", 2: "optimismSepolia", 26: "arc" }
const SOURCE_DOMAINS = { arc: 26, baseSepolia: 6, sepolia: 0, arbitrumSepolia: 3, optimismSepolia: 2 }
...
async function getAttestation(txHash, srcDomain) {
  const url = IRIS + "/v2/messages/" + (srcDomain || ARC_DOMAIN) + "?transactionHash=" + txHash
...
export default async function handler(req, res) {
  ...
  const reqSource = String((req.query && req.query.source) || (req.body && req.body.source) || "").trim()
  const srcDomain = SOURCE_DOMAINS[reqSource] || ARC_DOMAIN
  const att = await getAttestation(txHash, srcDomain)
  ...
  const attDom = (m.destinationDomain != null) ? m.destinationDomain : (m.decodedMessage && m.decodedMessage.destinationDomain)
  const destName = (reqDest && EVM_DESTS[reqDest]) ? reqDest : DOMAIN_TO_DEST[Number(attDom)]
  const dest = destName ? EVM_DESTS[destName] : null
  if (!dest) { res.status(400).json({ status: "unsupported_destination", detail: "destinationDomain " + attDom + " has no EVM completer target (pass ?dest=baseSepolia to override)", destinationDomain: attDom }); return }
