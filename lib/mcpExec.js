...
const ALLOW_EVM = ["0xd4939e42bd3e0ec9cb00091778a331c35d1834aa", "0x6829860b7f61FA01E5bf3D194d9f780ACa5B6787"].map(function (s) { return s.toLowerCase() })
...
export const PRIVATE_EXECUTE_TOOLS = [
  ...
  { name: "cronus_bridge_execute", description: "PRIVATE (auth-gated): execute a REAL CCTP bridge burn of Cronus treasury USDC on Arc or Base Mainnet. The server injects the Cronus exec secret from its own env; the caller never supplies a key. Recipient MUST be a Cronus treasury-owned allowlisted EVM address. Server-enforced caps: 5 USDC/bridge, 500 USDC/day bridge breaker, 1/min.", inputSchema: { type: "object", properties: { dest: { type: "string", enum: ["base", "arc"], description: "Destination chain: Base Mainnet or Arc Mainnet." }, source: { type: "string", enum: ["arc", "base"], description: "Source chain to burn from. Default arc. Use base for the Base->Arc reverse leg." }, to: { type: "string", description: "Recipient — MUST be a Cronus treasury allowlisted EVM address." }, amount: { type: "string", description: "Amount in USDC (decimal), e.g. 1.0." }, maxFee: { type: "string", description: "Optional max CCTP fee in USDC; default amount/100." } }, required: ["dest", "to", "amount"] } },
  { name: "cronus_bridge_complete", description: "PRIVATE (auth-gated): finalize a REAL Arc<->Base Mainnet CCTP bridge. Given the source-chain burn txHash from cronus_bridge_execute, the server fetches the Circle mainnet attestation and calls MessageTransmitterV2.receiveMessage on the destination chain. Moves no NEW funds; the server pays destination gas. Idempotent per burn txHash. Returns the real destination mintTxHash and explorer.", inputSchema: { type: "object", properties: { txHash: { type: "string", description: "The source-chain burn txHash returned by cronus_bridge_execute (0x + 64 hex)." }, dest: { type: "string", enum: ["base", "arc"], description: "Optional destination override; auto-resolved from the attestation if omitted." }, source: { type: "string", enum: ["arc", "base"], description: "Source chain of the burn. Default arc. Set base for a Base->Arc burn." } }, required: ["txHash"] } },
]
