...
async function demoGuardSwap(args) {
	const side = typeof args.side === "string" ? args.side : "usdc_to_crn"
	if (side !== "usdc_to_crn") return { endpoint: "/api/swap", httpStatus: 403, tier: "demo", error: "demo token: only usdc_to_crn swaps are allowed", _isError: true }
	const amt = Number(args.amountIn)
	if (!(amt > 0) || amt > DEMO_SWAP_MAX_ATOMIC) return { endpoint: "/api/swap", httpStatus: 403, tier: "demo", error: "demo token: amountIn exceeds cap (max " + DEMO_SWAP_MAX_ATOMIC + " atomic = 0.1 USDC)", _isError: true }
...
export async function callExecute(base, name, args, tier) {
  ...
  if (name === "cronus_swap_execute") {
		if (tier === "demo") { const g = await demoGuardSwap(args); if (g) return g }
  ...
  if (name === "cronus_bridge_complete") {
    ...
    const qs = "?txHash=" + encodeURIComponent(txHash) + (args.dest ? "&dest=" + encodeURIComponent(args.dest) : "")
