...
const RPC_URLS = [process.env.SIGNAL_RPC_URL, process.env.VITE_RPC_URL, process.env.RPC_URL, "https://rpc.mainnet.arc.io"].filter(Boolean)
...
async function payoutCapacityUsdc() {
  const rpcUrl = RPC_URLS[0] || "https://rpc.mainnet.arc.io"
  const data = "0x70a08231" + String(PAY_TO).replace(/^0x/, "").toLowerCase().padStart(64, "0")
  try {
    const r = await fetch(rpcUrl, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "eth_call", params: [{ to: USDC_ASSET, data }, "latest"] }) })
    const j = await r.json()
...
export default async function insurance(req, res) {
  ...
  try {
    if (action === "quote") {
      ...
      const quoteCapacity = await payoutCapacityUsdc()
      ...
      res.status(200).json({
        ...
        insurance_reserve_usdc: reserve, reserve_bps_of_premium: RESERVE_BPS, payout_capacity_usdc: quoteCapacity, payout_capacity_source: "on-chain balanceOf(treasury " + PAY_TO + ")",
        ...
        honest_note: "Demo insurance on testnet. Refunds honored from Cronus's self-operated treasury; payout_capacity_usdc is the live on-chain USDC balance of that treasury (verifiable on arcscan). insurance_reserve tracks a " + (RESERVE_BPS / 100) + "% claims buffer accrued from premiums (0 until real premiums are collected). No external underwriter.",
    ...
    if (action === "buy") {
      ...
      const policy = {
        ...
        payment: { network: NETWORK, txHash, block: proof.block, asset: USDC_ASSET, payTo: PAY_TO, explorer: "https://explorer.arc.io/tx/" + txHash },
    ...
    const reserve = await readReserve()
    const statusCapacity = await payoutCapacityUsdc(); res.status(200).json({ ok: true, policy, insurance_reserve_usdc: reserve, payout_capacity_usdc: statusCapacity, payout_capacity_source: "on-chain balanceOf(treasury " + PAY_TO + ")", claimable: policy.status === "refunded", state_changed: changed, miss_check: miss, note: "Status derived from on-chain /api/decisions + coverage window. Refund = full premium on MISS; reserve adjusted.", updatedAt: new Date().toISOString() })
  } catch (e) {
