...
export default async function handler(req, res) {
  ...
  if (action === "resolve") {
    ...
    if (req.method === "GET") return res.status(200).json({ ok: true, dryRun: true, due: preview, openCount: list.filter((x) => x.status === "OPEN").length })
    const auth = String(req.headers.authorization || "")
    if (!process.env.CRON_SECRET || auth !== "Bearer " + process.env.CRON_SECRET) return res.status(401).json({ ok: false, error: "auth required" })
