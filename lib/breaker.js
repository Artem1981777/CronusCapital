...
const DEFAULT_BRIDGE_DAILY = process.env.BRIDGE_DAILY_CAP_ATOMIC || "500000000"

function bridgeDayKey() {
  return "cronus:bridge:day:" + new Date().toISOString().slice(0, 10)
}

export async function checkBridgeDaily(amountAtomic) {
  const cap = BigInt(DEFAULT_BRIDGE_DAILY)
  const today = await kvCmd(["GET", bridgeDayKey()])
  if (!today.ok) {
    return {
      allowed: false,
      unavailable: true,
      reason: "bridge breaker store unavailable; per-call cap still applies",
      remainingAtomic: "0",
      dailyCapAtomic: String(cap),
    }
  }
  const spent = BigInt(today.result || "0")
  return Object.assign(
    {
      unavailable: false,
      spentTodayAtomic: String(spent),
      dailyCapAtomic: String(cap),
    },
    decideDaily(String(spent), amountAtomic, String(cap)),
  )
}

export async function recordBridgeDaily(amountAtomic) {
  const k = bridgeDayKey()
  await kvCmd(["INCRBY", k, String(amountAtomic)])
  await kvCmd(["EXPIRE", k, "172800"])
}
