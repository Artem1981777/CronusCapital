...
//   correct -> stake returned to the agent; wrong -> stake slashed to the protocol treasury.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({
      ...
      principle: "The agent puts real USDC at risk behind its own high-conviction verdicts. Every position is committed on-chain (keccak256) BEFORE the outcome is known, then resolved verifiably: correct -> stake returned to the agent, wrong -> stake slashed to the protocol treasury. This feed reports only real recorded positions and never backfills or fabricates a track record.",
      ...
      rules: {
        ...
        slash: "wrong -> full stake to protocol treasury; correct -> full stake returned to agent",
        commitment: "keccak256(verdict, conviction_bps, marketId, resolveBy, stakeAtomic, nonce) published on-chain before resolution",
      },
      escrow: ESCROW,
      treasury: TREASURY,
