...
// The gate above is the base level of caution, not a fixed verdict: it LEARNS from the
// record (see adaptiveGate). These bounds keep the learned gate sane; all env-overridable.
const GATE_FLOOR = Number(process.env.STAKE_GATE_FLOOR || "0.55")
const GATE_CAP = Number(process.env.STAKE_GATE_CAP || "0.80")
const ADAPT_MIN_RESOLVED = Number(process.env.STAKE_ADAPT_MIN_RESOLVED || "20")
const ADAPT_TARGET_ACCURACY = Number(process.env.STAKE_ADAPT_TARGET || "0.60")
const ADAPT_K = Number(process.env.STAKE_ADAPT_K || "0.50")
...
// pure: the conviction gate LEARNS from realized on-chain accuracy instead of being a bare
// constant. It moves ONLY once enough verdicts have actually resolved; below that it returns
// the base gate unchanged and says why, because two coin-flips cannot tell skill from luck.
// Deterministic and reproducible: same ledger in, same gate out, no randomness, no hidden state.
export function adaptiveGate(stats) {
  const st = stats || {}
  const resolved = Number(st.resolved_positions || st.resolved || 0) || 0
  const correct = Number(st.correct || 0) || 0
  const base = CONVICTION_GATE
  const accuracy = resolved > 0 ? correct / resolved : null
  const rule = "gate = clamp(base + (" + ADAPT_TARGET_ACCURACY + " - accuracy) * " + ADAPT_K + ", " + GATE_FLOOR + ", " + GATE_CAP + "): weaker realized accuracy demands more conviction before real USDC is risked; a stronger record earns a lower bar."
  if (resolved < ADAPT_MIN_RESOLVED) {
    return {
      gate: base, base, adaptive: false,
      reason: "insufficient_resolved_outcomes",
      resolved, min_resolved_to_adapt: ADAPT_MIN_RESOLVED,
      accuracy: accuracy == null ? null : Number(accuracy.toFixed(4)),
      floor: GATE_FLOOR, cap: GATE_CAP, rule,
    }
  }
  const raw = base + (ADAPT_TARGET_ACCURACY - accuracy) * ADAPT_K
  const gate = Number(Math.max(GATE_FLOOR, Math.min(GATE_CAP, raw)).toFixed(4))
  return {
    gate, base, adaptive: true,
    reason: "learned_from_resolved_outcomes",
    resolved, min_resolved_to_adapt: ADAPT_MIN_RESOLVED,
...
// pure: conviction (0..1) -> stake in atomic USDC. Below the (possibly learned) gate -> "0".
export function stakeAtomicForConviction(conviction, stats) {
  const c = Math.max(0, Math.min(1, Number(conviction) || 0))
  const gate = stats ? adaptiveGate(stats).gate : CONVICTION_GATE
  if (c < gate) return "0"
