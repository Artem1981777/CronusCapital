...
import { PRIVATE_EXECUTE_TOOLS, isPrivateAuthorized, tokenTier, callExecute } from "./mcpExec.js"
...
async function callTool(base, name, args, clientName, isPrivate) {
  if (name === "cronus_swap_execute" || name === "cronus_bridge_execute" || name === "cronus_bridge_complete") {
    if (!isPrivate) return { _unknown: true, error: "Unknown tool: " + String(name) }
    return await callExecute(base, name, args, isPrivate)
  }
...
export default async function mcp(req, res) {
  ...
  const kindPrivate = String((req.query && req.query.kind) || "").toLowerCase() === "mcp-private"
  const isPrivate = kindPrivate ? tokenTier(req) : null
  if (kindPrivate && !isPrivate) return res.status(401).json(errObj(null, -32001, "unauthorized: valid MCP token required"))
  let msg = req.body
