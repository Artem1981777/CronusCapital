...
const GATEWAY_NETWORK = process.env.GATEWAY_NETWORK || "eip155:5042"
const IDENTITY_REGISTRY = (process.env.IDENTITY_REGISTRY || "0x5B179bFF284a17a5C8C3ccaDed1984949B410522").toLowerCase()
const JOB_ESCROW = (process.env.JOB_ESCROW || "0x5C7f8f12D6aEcbA1A7461435F45f25b6c8Be18Ad").toLowerCase()
const REPUTATION_REGISTRY = (process.env.REPUTATION_REGISTRY || "0x7426bF7ec186F7E0fb57D3f9487fA94234D2C1Dd").toLowerCase()
...
export default function handler(req, res) {
  ...
  res.status(200).json({
    ...
    capabilities: {
      identity: { standard: "ERC-8004", registry: "https://explorer.arc.io/address/" + IDENTITY_REGISTRY, status: "live", note: "On-chain agent identity + reputation; sellers can gate by resolveByAddress()." },
    ...
    services: [{
      ...
      requires: "buyer holds a Circle Gateway USDC balance (one-time deposit into the Gateway Wallet)",
        ...
        verification: "premium verified on-chain via JSON-RPC (USDC transfer >= premium to payTo); MISS determined from the on-chain CronusDecisions log (/api/decisions). Live on Arc Mainnet; refunds honored from the self-operated treasury.",
      }],
