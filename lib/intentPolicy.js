// lib/intentPolicy.js — Intent Policy Decision Records (ipdr-3). ADDITIVE.
//
// WHAT THIS IS. A deterministic, multilingual front door for money intents. Text in any
// supported language is canonicalized WITHOUT an LLM into an allowlisted structure; money
// checks are delegated to lib/policyKernel.js; and EVERY decision — including every
// refusal — becomes a content-addressed Policy Decision Record that anyone can re-hash
// and verify, using the same canonicalization as lib/traceArchive.js.
//
// WHY NOT AN LLM. A model that reads the request can hallucinate a route, and its output
// is not reproducible, so a refusal cannot be audited after the fact. Here the same
// sentence in six languages produces a byte-identical decision, and the test suite proves
// it. No model output can reach the money path.
//
// WHY TOKENS, NOT \b. JavaScript's \b is defined over [A-Za-z0-9_], so a word boundary
// never fires around Cyrillic. Matching is done on Unicode-aware tokens instead.
//
// LANGUAGE IS A LABEL, NOT A GATE. The detected language is recorded for transparency and
// never changes the outcome. Spanish and Portuguese overlap heavily; a misattribution
// between them is cosmetic because both canonicalize to the same English structure.
//
// WHAT THIS IS NOT. Not a hash chain: records are content-addressed, not linked.
// Not custody: Cronus holds no keys; the visitor's own wallet signs every transaction.
// Not a DEX: the only convertible pair integrated is USDC<->USYC via the Teller on Arc.
// Every other pair is refused by name, never routed by guesswork.
//
