...
export default async function handler(req, res) {
  ...
  if (isStellar) {
    ...
    let plan
    try { plan = buildBurnArgs({ amountAtomic: String(amtAtomic), destChain: dest, recipient: to, maxFeeAtomic: String(maxFeeAtomic) }) }
  ...
  const execSecret = process.env.CRONUS_EXEC_SECRET || ""
  ...
  try {
    ...
    const sim = await publicClient.simulateContract({ account, address: ARC_TOKEN_MESSENGER, abi: burnAbi, functionName: fnName, args: burnArgs })
    ...
    return res.status(200).json({
      ...
      mint: isStellar
        ...
        : { status: "pending_attestation", note: "Burn on Arc verifiable on arcscan; USDC mint on " + dest + " completes after Circle CCTP attestation, not fabricated." },
    })
  } catch (e) {
    if (kvOn) await kvSet(RATE_KEY, "0")
