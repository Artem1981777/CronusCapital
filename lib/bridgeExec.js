...
//   - dest=<EVM chain name> (base or arc): depositForBurn; mint completes on
...
import { checkBridgeDaily, recordBridgeDaily } from "./breaker.js"
import { emergencyPaused, pauseError } from "./guard.js"

const ARC_USDC = "0x3600000000000000000000000000000000000000"
const ARC_TOKEN_MESSENGER = process.env.ARC_MAINNET_TOKEN_MESSENGER_ADDRESS || "0x28b5a0e9C621a5BadaA536219b3a228C8168cf5d"
const ARC_RPC = process.env.ARC_MAINNET_RPC || "https://rpc.mainnet.arc.io"
const ARCSCAN = process.env.ARC_EXPLORER || "https://explorer.arc.io/tx/"
const ARC_CHAIN_ID = Number(process.env.ARC_MAINNET_CHAIN_ID || 5042)
...
const ALLOW_EVM = String(process.env.TREASURY_ALLOWLIST || "0xd4939e42bd3e0ec9cb00091778a331c35d1834aa,0x6829860b7f61FA01E5bf3D194d9f780ACa5B6787").split(",").map(function (x) { return x.trim().toLowerCase() }).filter(Boolean)
...
const SOURCES = {
  ...
  base: {
    chainId: Number(process.env.BASE_MAINNET_CHAIN_ID || "8453"),
    rpc: process.env.BASE_MAINNET_RPC || "https://mainnet.base.org",
    usdc: process.env.BASE_MAINNET_USDC_ADDRESS || "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",
    tokenMessenger: process.env.BASE_MAINNET_TOKEN_MESSENGER_ADDRESS || "0x28b5a0e9C621a5BadaA536219b3a228C8168cf5d",
...
export default async function handler(req, res) {
  ...
  if (method === "GET") {
    return res.status(200).json({
      ...
      source: "Arc mainnet (CCTP domain 26)",
  ...
  if (!src) return res.status(400).json({ ok: false, error: "unsupported source chain: " + source + " (supported: arc, base)" })
  ...
  const gate = await checkBridgeDaily(String(amtAtomic))
  ...
  try {
    ...
    await recordBridgeDaily(String(amtAtomic)).catch(() => null)
