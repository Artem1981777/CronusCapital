...
export default function handler(req, res) {
  ...
  // Declared because they are verified. Every path below is exercised by
      ...
      "/api/insurance-quote": { get: { summary: "FREE insurance quote for a Cronus signal", parameters: [{ name: "instId", in: "query", required: false, schema: { type: "string" }, example: "ETH-USDC" }, { name: "notional", in: "query", required: true, schema: { type: "number" }, description: "Position notional in USDC to insure" }, { name: "topic", in: "query", required: false, schema: { type: "string" }, example: "ETH-USDC momentum" }], responses: { "200": { description: "Premium (5% of notional), money-back payout if MISS, 24h coverage window, current conviction/verdict, trace hash and reserve. No payment." } } } },
        "/api/insurance-buy": { get: { summary: "Buy signal insurance (x402; premium = 5% of notional)", description: "First request without payment returns HTTP 402 with x402 payment requirements for the premium. Pay the premium in USDC on " + NETWORK + " to payTo, then retry with the X-PAYMENT header set to your txHash.", parameters: [{ name: "notional", in: "query", required: true, schema: { type: "number" }, description: "Position notional in USDC to insure" }, { name: "instId", in: "query", required: false, schema: { type: "string" }, example: "ETH-USDC" }, { name: "topic", in: "query", required: false, schema: { type: "string" }, example: "ETH-USDC momentum" }, { name: "X-PAYMENT", in: "header", required: false, schema: { type: "string" }, description: "Arc txHash (0x + 64 hex) of the USDC premium payment to payTo" }], responses: { "402": { description: "Payment required (x402): premium quote in accepts[]." }, "200": { description: "Premium verified on-chain - policy issued: policy_id, coverage window, payout_on_miss." } } } },
        "/api/insurance-status": { get: { summary: "Insurance policy status + money-back settlement", parameters: [{ name: "policy_id", in: "query", required: true, schema: { type: "string" }, description: "Policy id returned by /api/insurance-buy" }], responses: { "200": { description: "Policy status; a MISS (Cronus conviction < 50 within coverage, verified on-chain via /api/decisions) yields a full-premium refund. Idempotent." } } } },
        "/api/cctp-status": { get: { summary: "Read-only Circle CCTP v2 attestation status (Arc domain 26 -> Stellar)", parameters: [{ name: "txHash", in: "query", required: false, schema: { type: "string" }, description: "Arc burn txHash (0x + 64 hex); omit for route info" }], responses: { "200": { description: "Attestation status (info | pending | complete | not_found) resolved via Circle IRIS. Read-only; never mints or moves funds." }, "400": { description: "Malformed txHash." } } } },
        "/api/identity": { get: { summary: "Read-only ERC-8004 agent identity (on-chain resolver, Arc testnet)", parameters: [{ name: "agentId", in: "query", required: false, schema: { type: "number" }, description: "Agent id (default 1)" }, { name: "address", in: "query", required: false, schema: { type: "string" }, description: "Resolve by agent address (0x + 40 hex)" }], responses: { "200": { description: "On-chain agent record from CronusIdentityRegistry via viem. Read-only; never writes or moves funds." }, "400": { description: "Malformed agentId/address." } } } },
        "/api/swap": { get: { summary: "Cronus AMM swap info + signer (read-only)", responses: { "200": { description: "Pool, pair, per-swap cap (2 USDC), daily cap, default slippage, auto-signal threshold, signer, configured flag. No funds move on GET." } } }, post: { summary: "Execute or dry-run a USDC/CRN swap (execKey-gated)", description: "Dry-run by default (live quote, minOut, executionPrice). A real on-chain swap requires execute:true and a valid execKey (Cronus exec secret); per-swap value capped at 2 USDC and 50/day, treasury-signed, 1/min. The MCP passes execKey through and never injects a server secret.", responses: { "200": { description: "Dry-run quote or executed swap: txHash, status, amountIn/Out, minOut, executionPrice." }, "401": { description: "Missing/invalid execKey for execute:true." }, "409": { description: "Rate limited (1/min) or cap exceeded." } } } },
        "/api/bridge": { get: { summary: "Cronus CCTP bridge info + signer (read-only)", responses: { "200": { description: "Source (Arc CCTP domain 26), EVM + Stellar destinations, per-bridge cap, rate limit, signer, dryRunDefault. No funds move on GET." } } }, post: { summary: "Execute or dry-run a CCTP bridge burn (execKey-gated)", description: "Dry-run by default (returns the burn plan). A real burn requires execute:true and a valid execKey (Cronus exec secret); per-bridge value capped (5 USDC default), shared daily breaker, 1/min. dest is stellar or an EVM chain name; to is the recipient (Stellar G-address or 0x EVM address); amount is USDC. Stellar mint completes via /api/cctp-status + /api/complete-stellar; EVM mint via Circle attestation relay. No destination hash is fabricated.", responses: { "200": { description: "Dry-run plan or executed burn: burnTx, explorer, signer, mint status." }, "400": { description: "Bad dest/recipient/amount or amount over cap." }, "401": { description: "Missing/invalid execKey for execute:true." }, "429": { description: "Rate limited (1/min) or daily breaker tripped." } } } },
