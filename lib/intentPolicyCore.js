...
export const VERIFIED_ROUTES = Object.freeze(["base>arc", "arc>base", "arbitrum>base", "ethereum>arc"])

export const TOKENS = Object.freeze(["usdc", "usyc", "crn"])
export const SWAP_PAIRS = Object.freeze(["usdc>usyc", "usyc>usdc", "usdc>crn", "crn>usdc"])

// Parsed is not the same as executable. USYC is a permissioned token and the Arc
// entitlements contract answers false for us, so that pair is understood and then
// refused on-chain rather than pretended. CRN trades against our own deployed pool.
export const TRADEABLE_PAIRS = Object.freeze(["usdc>crn", "crn>usdc"])
...
function parseSwap(t, lang) {
  ...
  const named = []
  for (const m of t.matchAll(/\b(usdc|usyc|crn)\b/g)) named.push(m[1])

  let fromToken = null, toToken = null
  if (named.length >= 2) { fromToken = named[0]; toToken = named[1] }
  else if (named.length === 1) {
    if (named[0] !== "usdc") {
      if (REDEEM_VERBS.test(t)) { fromToken = named[0]; toToken = "usdc" }
      else { fromToken = "usdc"; toToken = named[0] }
  ...
  return Object.freeze({
    ...
    pairTradeable: TRADEABLE_PAIRS.includes(fromToken + ">" + toToken),
  })
}
...
export function parseIntent(text) {
  ...
  if (/\bmainnet\b/.test(t) && !isBridge) {
    return refusal(["mainnet_not_supported"], [], lang)
  }
