...
export default async function handler(req, res) {
  ...
  try {
    let data
    try { data = await scanExplorer() } catch { try { await new Promise((r2) => setTimeout(r2, 900)); data = await scanExplorer() } catch { data = await scanKnown() } }
    ...
    res.status(200).json({
      ...
      lastTx: data.lastTx,
      explorer: data.lastTx ? ("https://explorer.arc.io/tx/" + data.lastTx) : null,
      source: data.source,
