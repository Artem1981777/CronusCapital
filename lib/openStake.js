...
import { stakeAtomicForConviction, adaptiveGate, readStakeLedger, reduceTrackRecord } from "./stake.js"
import { latestSignalBuyer } from "./upgrades/makegoodEscrow.js"

const ARC_CHAIN_ID = Number(process.env.ARC_CHAIN_ID || "5042002")
const ARC_RPC = process.env.ARC_RPC || process.env.ARC_RPC_URL || "https://rpc.mainnet.arc.io"
...
const STAKE_TREASURY_EXPECT = (process.env.STAKE_TREASURY || "0x46213abeca58cc9a89a269fd25a8737c700ca164").toLowerCase()
...
const arcChain = defineChain({ id: ARC_CHAIN_ID, name: "arc-mainnet", nativeCurrency: { name: "USDC", symbol: "USDC", decimals: 18 }, rpcUrls: { default: { http: [ARC_RPC] } } })
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET" && req.query && req.query.check === "signer") {
    const pk0 = normPk(process.env.STAKE_PRIVATE_KEY || process.env.TREASURY_PRIVATE_KEY)
    if (!pk0) { res.status(500).json({ ok: false, error: "no staking key (set STAKE_PRIVATE_KEY)" }); return }
    try {
      const acct = privateKeyToAccount(pk0)
      const pub = createPublicClient({ chain: arcChain, transport: http(ARC_RPC) })
      let native = "0", usdc = "0"
      try { native = String(await pub.getBalance({ address: acct.address })) } catch (_) {}
      try { usdc = String(await pub.readContract({ address: USDC_ADDRESS, abi: erc20Abi, functionName: "balanceOf", args: [acct.address] })) } catch (_) {}
      res.status(200).json({ ok: true, signer: acct.address, expected: STAKE_TREASURY_EXPECT, match: acct.address.toLowerCase() === STAKE_TREASURY_EXPECT, nativeWei: native, usdc6: usdc })
  ...
  try {
    ...
    // Close the loop honestly: the threshold that gates real USDC is the one the agent
    // earned from its own resolved record, read live. If the ledger is unreachable the base
    // gate applies - a money-moving action must never fail because learning input was down.
    let learnStats = null
    try { learnStats = reduceTrackRecord(await readStakeLedger()) } catch (_) { learnStats = null }
    const gateInfo = adaptiveGate(learnStats || {})
    const gateApplied = gateInfo.gate
    const stakeAtomic = stakeAtomicForConviction(conviction01, learnStats || {})
    ...
    if (STAKE_TREASURY_EXPECT && account.address.toLowerCase() !== STAKE_TREASURY_EXPECT) { res.status(409).json({ ok: false, error: "staking signer " + account.address + " != expected " + STAKE_TREASURY_EXPECT + " — set STAKE_PRIVATE_KEY for the intended wallet" }); return }
    ...
    // ADDITIVE (make-good): bind this stake to the most recent REAL buyer of a signal on the
    // same instrument, so a WRONG resolution compensates them instead of burning. No buyer -> burn.
    let slashPolicy, beneficiary
    try { const bmg = await latestSignalBuyer(pick.instId); if (bmg) { beneficiary = bmg; slashPolicy = "makegood" } } catch (_) {}
    const position = {
      id: commitment.slice(0, 18), marketId, verdict: pick.verdict, conviction: conviction01, convictionBps, gateApplied,
      ...
      slashPolicy: slashPolicy || "burn", beneficiary: beneficiary || null,
    }
    const len = await kvCmd(["LPUSH", "cronus:stakes:ledger", JSON.stringify(position)])
    res.status(200).json({ ok: true, opened: true, position, learning: gateInfo, ledgerLength: len, explorer: EXPLORER + "/tx/" + hash })
  } catch (e) {
