...
const GUARD = (process.env.AGENT_GUARD_V2 || "0x37846B767EC1889b0b719945c1168e42228b944C").toLowerCase()
const MULTISIG = (process.env.CRONUS_MULTISIG || "0xde8874C53D82a38c1c2864ea575f9E62Dc29dA5F").toLowerCase()
const EXPLORER_ADDR = "https://explorer.arc.io/address/"
...
const PENDING_SCAN = Number(process.env.GOVERNANCE_PENDING_SCAN || "24")
const CONCURRENCY = 4
...
const GUARD_ABI = [
  ...
  "function available() view returns (uint256)",
	...
	"function queue(bytes data, bytes32 salt) returns (bytes32)",
	"function extSetGuardian(address)",
	"function extSetOperator(address)",
	"function extSetLimits(uint256,uint256)",
	"function extAddAllowed(address)",
	"function extTransferOwnership(address)",
	"function extRenounceOwnership()",
]
const MS_ABI = [
...
export async function resolveGovernance() {
  ...
  const txRows = await mapPool(ids, CONCURRENCY, async (i) => {
    ...
    return { id: i, to: addr(t[0]), value: String(t[1]), executed: !!t[3], confirmations: num(t[4]), raw: data, selector: data.slice(0, 10), dataBytes: Math.max(0, (data.length - 2) / 2), targetsGuard: addr(t[0]) === GUARD }
  })
  const txsSeen = txRows.filter(Boolean)
  const pending = txsSeen.filter((t) => !t.executed).map((t) => {
		const rest = { ...t }
		delete rest.raw
		return { ...rest, confirmationsNeeded: threshold === null || t.confirmations === null ? null : Math.max(0, threshold - t.confirmations) }
	})

	// A rules change does not live in the multisig once its transaction executed: it lives
	// in guard.opEta, keyed by keccak256(abi.encode(data, salt)). Both arguments are inside
	// the executed multisig call, so the queue is read exactly, with no log scan to fail.
	const QUEUE_SEL = GI.getFunction("queue").selector
	const nowSec = Math.floor(Date.now() / 1000)
	const timelockQueued = []
	for (const t of txsSeen) {
		if (!t.targetsGuard || !t.executed || t.selector !== QUEUE_SEL) continue
		let inner = null
		let salt = null
		try {
			const dec = GI.decodeFunctionData("queue", t.raw)
			inner = String(dec[0])
			salt = String(dec[1])
		} catch (_) { continue }
		const opId = keccak256(AbiCoder.defaultAbiCoder().encode(["bytes", "bytes32"], [inner, salt]))
		const eta = num(await r.call(GI, GUARD, "opEta", [opId], "guard.opEta(" + opId.slice(0, 10) + ")"))
		if (eta === null || eta === 0) continue
		let action = inner.slice(0, 10)
		try {
			const parsed = GI.parseTransaction({ data: inner })
			if (parsed && parsed.name) action = parsed.name
		} catch (_) {}
		timelockQueued.push({ id: opId, action, calldata: inner, salt, queuedByMultisigTx: t.id, eta, etaIso: new Date(eta * 1000).toISOString(), secondsRemaining: Math.max(0, eta - nowSec), executable: nowSec >= eta })
	}
	const guardianFix = timelockQueued.find((q) => q.action === "extSetGuardian") || null
  ...
  const invariants = [
    ...
    inv("the spend role and the pause role are held by different keys", operatorAddr === null || guardianAddr === null ? null : operatorAddr !== guardianAddr, operatorAddr === guardianAddr ? "one key is both operator and guardian" : "separate keys"),
		inv("a rules change in flight is disclosed with the exact time it can execute", timelockQueued.every((q) => q.eta !== null && q.secondsRemaining !== null), timelockQueued.length === 0 ? "no rules change in flight" : timelockQueued.length + " queued, earliest execution in " + Math.min.apply(null, timelockQueued.map((q) => q.secondsRemaining)) + "s"),
  ]
  ...
  if (operatorAddr !== null && guardianAddr !== null && operatorAddr === guardianAddr) knownGaps.push({ gap: "one key holds both the spend role and the pause role", impact: "the documented three-role split is two roles on-chain, so the guardian is not an independent circuit breaker against the operator", severity: "medium", fix: "queue extSetGuardian to a separate cold watcher key", remediation: guardianFix === null ? null : { status: guardianFix.executable ? "queued, timelock expired, awaiting execution" : "queued, waiting out the timelock", action: guardianFix.action, executableAt: guardianFix.etaIso, secondsRemaining: guardianFix.secondsRemaining, queuedByMultisigTx: guardianFix.queuedByMultisigTx, opId: guardianFix.id, note: "The fix is on-chain and time-stamped, not a promise in prose: it cannot execute before executableAt, and the guardian or the owner can still cancel it." } })
  return {
    ok: true,
    resolver: "cronus-governance",
    generatedAt: new Date().toISOString(),
    network: { name: "arc-mainnet", chainId: Number(process.env.ARC_CHAIN_ID || 5042), rpc: RPC },
    guard: {
      address: GUARD,
    ...
    invariants,
    		timelock: {
			delaySeconds: delay,
			queuedCount: timelockQueued.length,
			queued: timelockQueued,
			scanned: "the last " + PENDING_SCAN + " multisig transactions",
			note: "Queued operations are read from guard.opEta, keyed by keccak256(abi.encode(data, salt)) recovered from the executed multisig call. An operation older than the scanned window would be missed, so the window is stated rather than implied.",
		},
		knownGaps,
    unread: r.unread,
