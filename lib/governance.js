...
export async function resolveGovernance() {
  ...
  const invariants = [
    ...
    inv("the agent hot key holds no governance power", operatorIsSigner === null ? null : operatorIsSigner === false, "operator is a multisig signer: " + operatorIsSigner),
  ]
  ...
  return {
    ...
    honesty: "Every field is read live from Arc with eth_call and no keys. A value the node refused is listed under unread and its invariant reads unknown, never satisfied.",
  }
}
