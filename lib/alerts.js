...
export default async function handler(req, res) {
  ...
  const settleList = arr(settle, ["rails.directOnchain.settlements"]) || []
  const lastSettle = settleList.length ? settleList[0] : null
  ...
  const snap = {
    ...
    lastSettleTx: lastSettle ? lastSettle.txHash : null,
    lastSettleUsdc: lastSettle ? lastSettle.amountUsdc : null,
    gatewayTransfers: num(settle, ["rails.gatewayBatched.onchainSettleTransfers"]),
    vaultNav: lastNav,
    spendRemaining: num(spend, ["remainingDailyAtomic"]) != null ? num(spend, ["remainingDailyAtomic"]) / (num(spend, ["scale"]) || 1000000) : null,
    spentToday: num(spend, ["spentTodayAtomic"]) != null ? num(spend, ["spentTodayAtomic"]) / (num(spend, ["scale"]) || 1000000) : null,
  ...
  if (snap.lastSettleTx && prev.lastSettleTx && snap.lastSettleTx !== prev.lastSettleTx) a.push("🌉 <b>New on-chain settlement!</b> " + (snap.lastSettleUsdc != null ? snap.lastSettleUsdc + " USDC, " : "") + "tx " + String(snap.lastSettleTx).slice(0, 12) + "…")
  if (grew("gatewayTransfers") && (!prev.gwAlertAt || nowMs - prev.gwAlertAt > 3600000)) a.push("🌊 <b>Nano-stream activity</b> — " + snap.gatewayTransfers + " Gateway transfer(s) in the recent window")
  ...
  if (grew("spentToday")) a.push("🤖 <b>Agent spent USDC upstream</b> — +" + (snap.spentToday - prev.spentToday).toFixed(6) + " USDC (today: " + snap.spentToday + ")")
  ...
  await kvSet("alerts:last", {
    ...snap,
    gwAlertAt: a.some((x) => x.indexOf("🌊") === 0) ? nowMs : (prev.gwAlertAt || 0),
