...
const RATE_KEY = "cronus:swap:ratelock"
...
export default async function handler(req, res) {
  ...
  if (amtAtomic > PER_SWAP_CAP_ATOMIC) return res.status(400).json({ ok: false, error: "amountIn exceeds per-swap cap", capAtomic: String(PER_SWAP_CAP_ATOMIC), requestedAtomic: String(amtAtomic) })
  ...
  const lock = await kvLock(RATE_KEY, 60)
  if (kvOn && lock !== "OK") return res.status(409).json({ ok: false, error: "rate limited: 1 swap per minute" })
