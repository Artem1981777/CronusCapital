...
const RPC = process.env.VITE_RPC_URL || process.env.SIGNAL_RPC_URL || process.env.ARC_RPC || process.env.RPC_URL || "https://rpc.mainnet.arc.io"
...
export default async function balance(req, res) {
  ...
  res.status(200).json({ ok: true, network: "arc-mainnet", address, usdc_balance: Number(usdcAtomic) / 1e6, usdc_atomic: usdcAtomic.toString(), usdc_native_18dp: Number(nativeWei) / 1e18, usdc_native_wei: nativeWei.toString(), native_note: "On Arc, USDC is the native gas token; usdc_balance (6dp ERC-20 balanceOf) and usdc_native_18dp (eth_getBalance/1e18) are the same value in different units.", last_payout_at, usdc_source: usdcErr ? ("error: " + usdcErr) : "eth_call balanceOf(" + USDC + ")", native_source: nativeErr ? ("error: " + nativeErr) : "eth_getBalance", updatedAt: new Date().toISOString() })
}
