...
export default async function handler(req, res) {
	...
	try {
		...
		res.status(200).json({
				external_payers: onchain ? onchain.onchain_external_payers : 0,
				external_txs: onchain ? onchain.onchain_external_txs : 0,
				external_usdc: onchain ? onchain.onchain_external_usdc : 0,
				canonical_field: "external_payers",
				headline_note: "external_payers is the canonical on-chain metric (distinct USDC payers to treasury minus our infra/demo wallets). nano.unique_external_payers tracks ONLY the separate nano KV ledger and stays 0 until a nano-tier external payer appears.",
			ok: true,
			...
			standard: standard ? { payments: standard.payments, totalUsdc: standard.totalUsdc, lastTx: standard.lastTx, source: standard.source } : null,
					onchain: onchain ? { onchain_external_payers: onchain.onchain_external_payers, onchain_external_txs: onchain.onchain_external_txs, onchain_external_usdc: onchain.onchain_external_usdc, onchain_self_txs: onchain.onchain_self_txs, note: "external = distinct USDC payers to treasury minus our infra/demo wallets; verify on Arc explorer" } : null,
