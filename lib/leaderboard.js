...
import { leaderboard, reduceClients, readNanoLedger, TREASURY, selfAddresses, reduceOnchainPayers, readReceipts, verifiedExternal, verifiedExternalList, readMcpEvents, reduceConnected } from "./traction.js"
...
export default async function handler(req, res) {
	...
	try {
		...
		const ledger = await readNanoLedger()
			const mcpEvents = await readMcpEvents()
		...
		const allow = new Set(verifiedExternalList().map((a) => String(a).toLowerCase()))
		...
		res.status(200).json({
			...
			ok: true,
			treasury: TREASURY,
			unique_external_payers: [...extSeen].filter((p) => allow.has(p)).length,
			unique_self_demo_payers: [...extSeen].filter((p) => !allow.has(p)).length,
			...
			self_demo_calls: selfDemo,
				recent_clients: reduceClients(ledger, { limit }),
					connected_clients: reduceConnected(mcpEvents, { limit }),
			count: rows.length,
			leaders: rows,
