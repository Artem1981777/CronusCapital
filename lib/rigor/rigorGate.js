...
export function edgeVerdict(w) {
  ...
  if (!Number.isFinite(sr) || !Number.isFinite(osr)) throw new Error("edgeVerdict требует inSampleSharpeRaw и oosSharpeRaw (Sharpe за период)")
...
export function makeRigorGate(factoryOpts) {
  ...
  return async function handler(req, res) {
    ...
    if (!bars.length) return res.status(200).json(Object.assign({}, base, { ok: false, reason: "candles_unavailable", edgeClaim: "unproven", dataProvenance: { synthetic: false, sources: ["okx:history-candles"], note: "OKX не отдал свечи; заявлять edge нельзя." } }))
    if (bars.length < MIN_BARS) return res.status(200).json(Object.assign({}, base, { ok: false, reason: "insufficient_bars", bars: bars.length, required: MIN_BARS, edgeClaim: "unproven", dataProvenance: { synthetic: false, sources: ["okx:history-candles"], note: "Истории меньше " + MIN_BARS + " баров: статистика недостоверна." } }))
    ...
    return res.status(200).json(Object.assign({}, base, {
      ...
      relationToKelly: "edge в /api/kelly (b*p - q) считается по трек-рекорду и НЕ поправлен на перебор. Здесь edgeClaim — единственное поле, на которое можно ссылаться как на заявление о преимуществе.",
      units: "inSampleSharpe/oosSharpe годовые (x sqrt(8760)) и приведены для чтения. Deflated Sharpe считается на Sharpe за период: смешение единиц даёт ложное proven-oos.",
      disclosure: "Порог выбран перебором на in-sample; Deflated Sharpe штрафует за число испытаний, OOS-часть подбор не видела. edgeClaim=unproven означает, что публично заявлять преимущество нельзя.",
      dataProvenance: { synthetic: false, sources: ["okx:history-candles"], computation: "deterministic_walk_forward", note: "Свечи OKX, детерминированный расчёт: один и тот же вход даёт один и тот же выход." },
    }))
  }
}
