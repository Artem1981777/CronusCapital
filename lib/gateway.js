...
async function scan(topics, windows, tip, deadlineAt) {
  ...
  const results = await mapPool(ranges, SCAN_CONCURRENCY, async (r) => {
    ...
    try {
      ...
      if (settled(r)) await kvCmd(["SET", memoKey(r), JSON.stringify(logs)])
...
export async function resolveSettlements(opts) {
  ...
  const direct = await resolveDirectSettlements(windows, tip, deadline)
  const gateway = await resolveGatewayFootprint(windows, tip, deadline)
  const out = {
    ok: true,
