...
const TREASURY = (process.env.CRONUS_PAYTO || "0xd4939e42bd3e0ec9cb00091778a331c35d1834aa").toLowerCase()
...
// A direct x402 settlement is a transfer of a published price to the treasury, not any
// inbound USDC. The treasury doubles as the AMM buyer and the bridge recipient, so swap
// proceeds and a CCTP mint were counted as revenue: three transfers inflated this rail to
// 12.150098 USDC when signals had actually earned 0.16. Amounts are now matched against
// the prices the manifest publishes; anything else is disclosed, never counted.
const PRICES = (process.env.X402_PRICES_USDC || "0.02,0.001").split(",").map((x) => Number(x.trim())).filter((n) => Number.isFinite(n) && n > 0)
const isPrice = (a) => PRICES.some((p) => Math.abs(Number(a) - p) < 1e-9)
const classify = (t) => (isZero(String(t.payer || "")) ? "bridge_mint" : isPrice(t.amountUsdc) ? "x402_price_match" : "unclassified_inbound")
...
// A settled window can never gain another log, so it is remembered instead of re-read.
// This is what makes a complete scan affordable: after the warmer has walked the range
// once, the node is asked for the head window only, so it stops refusing.
const LOG_MEMO_PREFIX = "cronus:gw:logs:"
const REORG_MARGIN = Number(process.env.SETTLEMENTS_REORG_MARGIN || "200")
// A settled window is immutable, so this expiry is storage hygiene, not freshness: it
// only bounds how many keys accumulate as the grid head walks forward.
const MEMO_TTL_SECONDS = Number(process.env.SETTLEMENTS_MEMO_TTL || "2592000")
const RETRY_BUDGET_MS = Number(process.env.SETTLEMENTS_RETRY_BUDGET_MS || "16000")
// One deadline for the whole request. Budgeting per rail let two rails plus the base
// scan add up past the platform ceiling, and a timeout is the one answer that helps
// nobody: no counts, no disclosure, and every dependent check red.
const REQUEST_BUDGET_MS = Number(process.env.SETTLEMENTS_REQUEST_BUDGET_MS || "38000")
...
async function scan(topics, windows, tip, deadlineAt) {
  ...
  const ranges = []
  // Windows are aligned to a fixed block grid instead of counted back from the moving
  // tip. Arc advances several blocks a second, so tip-relative ranges never repeated and
  // the memo could never hit: every request re-asked the node for all fifty windows and
  // was refused again. Grid-aligned ranges below the head window are immutable and
  // identical on every request, which is what makes remembering them possible.
  const head = Math.floor(latest / RANGE) * RANGE
  for (let i = 0; i < windows; i++) {
    const fr = head - i * RANGE
    if (fr < 0) break
    const to = i === 0 ? latest : fr + RANGE - 1
        ranges.push({ to, fr: Math.max(0, fr) })
  }
  const errs = []
  const tag = topics.map((t) => (t ? String(t).slice(-8) : "x")).join("-")
  const memoKey = (r) => LOG_MEMO_PREFIX + tag + ":" + r.fr + "-" + r.to
  const deadline = Number.isFinite(deadlineAt) ? deadlineAt : Date.now() + RETRY_BUDGET_MS
  const settled = (r) => r.to < latest - REORG_MARGIN
  const fetchWindow = async (r) => {
    if (Date.now() > deadline) throw new Error("request budget exhausted before this window could be read")
    const res = await rpc("eth_getLogs", [{ address: USDC, topics, fromBlock: hx(r.fr), toBlock: hx(r.to) }])
    return (Array.isArray(res) ? res : []).map((l) => ({
  ...
  const results = await mapPool(ranges, SCAN_CONCURRENCY, async (r) => {
    if (settled(r)) {
      const memo = await kvCmd(["GET", memoKey(r)])
      if (memo) {
        try { const p = typeof memo === "string" ? JSON.parse(memo) : memo; if (Array.isArray(p)) return { logs: p, memo: true } } catch (_) {}
      }
    }
    try {
      const logs = await fetchWindow(r)
      if (settled(r)) await kvCmd(["SET", memoKey(r), JSON.stringify(logs), "EX", String(MEMO_TTL_SECONDS)])
      return { logs, memo: false }
    } catch (e) { errs.push(String((e && e.message) || e)); return null }
  })
  // A refused window is not an empty window. Refused windows are re-asked one at a time,
  // with a longer pause and a hard time budget, so a slow node degrades into a disclosed
  // partial scan rather than a confident wrong total.
  // bounded by the shared deadline computed above
  for (let i = 0; i < results.length; i++) {
    if (results[i]) continue
    if (Date.now() > deadline) break
    const r = ranges[i]
    for (let att = 0; att < 2 && !results[i]; att++) {
      if (Date.now() > deadline) break
      await new Promise((z) => setTimeout(z, 450 + att * 850))
      try {
        const logs = await fetchWindow(r)
        if (settled(r)) await kvCmd(["SET", memoKey(r), JSON.stringify(logs), "EX", String(MEMO_TTL_SECONDS)])
        results[i] = { logs, memo: false }
  ...
  for (const r of results) {
    ...
    if (r.memo) memoHits++
    for (const l of r.logs) out.push(l)
  }
  return { latest, logs: out, windowsRequested: ranges.length, windowsFailed: failed, windowsMemoized: memoHits, windowErrors: Array.from(new Set(errs)).slice(0, 3) }
}
...
export async function resolveDirectSettlements(windows, tip, deadlineAt) {
  let anchorsPre = []
  try { anchorsPre = await resolveAnchorSettlements() } catch (e) { anchorsPre = [] }
  const { latest, logs, windowsRequested, windowsFailed, windowsMemoized, windowErrors } = await scan([TRANSFER_TOPIC, null, pad(TREASURY)], windows, tip, deadlineAt)
  const scanned = logs
  ...
  for (const t of settlements) t.classification = classify(t)
  const payments = settlements.filter((t) => t.classification === "x402_price_match")
  const nonPayments = settlements.filter((t) => t.classification !== "x402_price_match")
  return {
    rail: "x402-exact",
    mapping: "1:1-onchain",
    chainTip: latest,
    ...
    scan: { windowsRequested, windowsFailed, windowsMemoized, complete: windowsFailed === 0, errors: windowErrors },
    totalUsdc: Number(payments.reduce((s, x) => s + x.amountUsdc, 0).toFixed(6)),
    settlements: payments.slice(0, 50),
    prices: PRICES,
    nonPayments: {
      count: nonPayments.length,
      totalUsdc: Number(nonPayments.reduce((s, x) => s + x.amountUsdc, 0).toFixed(6)),
      note: "Inbound USDC transfers to the treasury that are not x402 payments: bridge mints, AMM swap proceeds and funding. The treasury address doubles as the AMM buyer and the bridge recipient, so a rail that counted every inbound transfer as a settlement reported 12.150098 USDC where signals had earned 0.16. Disclosed here rather than dropped, and excluded from count and totalUsdc.",
      transfers: nonPayments.slice(0, 20),
    },
  }
}

export async function resolveGatewayFootprint(windows, tip, deadlineAt) {
  const { latest, logs, windowsRequested, windowsFailed, windowsMemoized, windowErrors } = await scan([TRANSFER_TOPIC, pad(GATEWAY_WALLET)], windows, tip, deadlineAt)
...
export async function resolveSettlements(opts) {
  ...
  // The rails run in turn and shared one deadline, so the first one spent all of it while
  // filling its memo and the batched rail was reached with nothing left: every window of
  // it failed on my own budget error, so it could never remember a single window and was
  // starved on every request. Each rail now gets its own ceiling. A deadline only caps a
  // rail, it reserves nothing, so once the first is warm the second inherits the slack.
  const halfway = started + Math.round(REQUEST_BUDGET_MS * 0.5)
  const direct = await resolveDirectSettlements(windows, tip, halfway)
  const gateway = await resolveGatewayFootprint(windows, tip, started + REQUEST_BUDGET_MS)
  const out = {
    ok: true,
