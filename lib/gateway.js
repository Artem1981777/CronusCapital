...
async function scan(topics, windows, tip) {
  ...
  return { latest, logs: out, windowsRequested: ranges.length, windowsFailed: failed, windowsMemoized: memoHits }
}
...
export async function resolveDirectSettlements(windows, tip) {
  ...
  const { latest, logs, windowsRequested, windowsFailed, windowsMemoized } = await scan([TRANSFER_TOPIC, null, pad(TREASURY)], windows, tip)
  const scanned = logs
  ...
  return {
    ...
    scan: { windowsRequested, windowsFailed, windowsMemoized, complete: windowsFailed === 0 },
...
export async function resolveGatewayFootprint(windows, tip) {
  const { latest, logs, windowsRequested, windowsFailed, windowsMemoized } = await scan([TRANSFER_TOPIC, pad(GATEWAY_WALLET)], windows, tip)
