...
// RPC fallback: when the explorer is down, re-verify the tx directly on Arc RPC.
const RPC_URLS = ["https://rpc.testnet.arc.network", process.env.ARC_RPC, process.env.SIGNAL_RPC_URL].filter(Boolean)
...
async function rpcCall(method, params) {
  ...
  for (const u of RPC_URLS) {
    try {
      ...
      lastErr = new Error(method + ": " + JSON.stringify(j && j.error))
    } catch (e) { lastErr = e }
  }
  throw lastErr || new Error("all RPCs failed")
}
async function verifyViaRpc(tx) {
  const rec = await rpcCall("eth_getTransactionReceipt", [tx])
  if (!rec || rec.status !== "0x1" || !Array.isArray(rec.logs)) return null
  for (const l of rec.logs) {
    if (String(l.address || "").toLowerCase() !== USDC) continue
    if (!l.topics || String(l.topics[0] || "").toLowerCase() !== TRANSFER_TOPIC || l.topics.length < 3) continue
    if (("0x" + String(l.topics[2]).slice(26)).toLowerCase() !== PAY_TO) continue
    const amountAtomic = parseInt(l.data, 16)
    const priceMatches = String(amountAtomic) === PRICE
    return { verified: priceMatches, priceMatches, payer: ("0x" + String(l.topics[1]).slice(26)).toLowerCase(), amountAtomic, block: rec.blockNumber ? parseInt(rec.blockNumber, 16) : null }
  }
  return null
}
export default async function handler(req, res) {
    ...
    let rpcProof = null
    if (!row) { try { rpcProof = await verifyViaRpc(tx) } catch (e) { rpcProof = null } }
    if (rpcProof) {
      res.setHeader("content-type", "application/json")
      res.setHeader("cache-control", "s-maxage=60, stale-while-revalidate=300")
      res.status(200).json({
        ok: true,
        verified: rpcProof.verified,
        txHash: tx,
        ...
        payer: rpcProof.payer,
        payTo: PAY_TO,
        amountAtomic: rpcProof.amountAtomic,
        amountUsdc: rpcProof.amountAtomic / 1000000,
        priceMatches: rpcProof.priceMatches,
        toTreasury: true,
        isUsdc: true,
        block: rpcProof.block,
        settled: true,
        source: "onchain-rpc",
        note: "explorer unavailable; re-verified directly on Arc RPC via eth_getTransactionReceipt (USDC Transfer log to the treasury at the exact x402 price)",
        commitment: k ? k.commitment : null,
        memoId: k ? k.memoId : null,
        nonCustodial: true,
        custodyNote: "Spend-path settlement is signed in the payer wallet; the agent holds no key on the buy side.",
