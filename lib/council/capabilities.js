...
export const ENDPOINTS = [
  ...
  { path: "/api/makegood", kind: "makegood", aliases: ["make-good"],
    what: "Read-only inspector for make-good slash routing: shows where each staked position's slashed amount would go.",
    params: {},
    returns: ["policy", "burnSink", "total", "makegoodPositions", "positions[].slashPolicy", "positions[].beneficiary", "positions[].slashGoesTo", "positions[].stakeAtomic"],
    refusals: ["ledger_unreachable", "kv_not_configured"],
    honesty: "Positions are read from kv:cronus:stakes:ledger; nothing is simulated and no funds move on GET. A position only compensates a real signal buyer when it explicitly carries slashPolicy:makegood AND a valid non-zero, non-burn beneficiary - any doubt routes back to the burn sink rather than inventing a recipient. makegoodPositions:0 means the live ledger currently holds no such position. Settlement itself happens in /api/resolve-stake (POST, Bearer CRON_SECRET), never here." },
