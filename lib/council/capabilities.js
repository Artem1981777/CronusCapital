...
export const ENDPOINTS = [
  ...
  { path: "/api/rigor", kind: "rigor", aliases: ["alpha-passport"],
    what: "Статистический гейт заявления об edge: walk-forward, Deflated Sharpe, PBO на реальных свечах OKX.",
    params: { instId: "инструмент OKX, по умолчанию BTC-USDT", bar: "таймфрейм, по умолчанию 1H", limit: "сколько баров тянуть" },
    ...
    honesty: "Порог подбирается только на in-sample; OOS-часть подбор не видела. Deflated Sharpe штрафует за число испытаний, а отложенная выборка обязана быть значимой сама по себе (t >= 2): на шуме получено DSR 0.99 при t 1.23, поэтому одного DSR недостаточно. edgeClaim=unproven — запрет публично заявлять преимущество; поле edge в /api/kelly поправки на перебор НЕ содержит." },
