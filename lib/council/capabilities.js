...
export const ENDPOINTS = [
  ...
  { path: "/api/treasury-yield", kind: "treasury-yield", aliases: [],
    ...
    honesty: "yieldFromChain is computed only from NAV points read on-chain; oracle rounds that fail validation are counted in roundsRejected rather than silently dropped. entitlement.canCall is false because USYC is permissioned, so idleTreasuryUsdc is labeled counterfactual and never touches vault NAV. When Arc cannot be read the route answers 502 rather than ok:true with nulls, or serves the last good read clearly marked stale with its real age." },
