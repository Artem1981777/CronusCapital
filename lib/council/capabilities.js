...
export const ENDPOINTS = [
  ...
  { path: "/api/treasury-yield", kind: "treasury-yield", aliases: [],
    what: "USYC money-market yield computed from on-chain NAV growth on Arc, cross-checked against the ERC-4626 teller's conversion. Cronus holds no USYC and proves why rather than claiming a position.",
    params: { days: "horizon in days, 1..365, defaults to 30", fresh: "1 - re-read the chain now instead of the ~15 minute cache" },
    returns: ["nav", "yieldFromChain", "roundsUsed", "roundsRejected", "entitlement", "idleTreasuryUsdc", "projection", "honesty", "verify", "cache"],
    refusals: ["arc_rpc_unavailable"],
    honesty: "yieldFromChain is computed only from NAV points read on-chain; oracle rounds that fail validation are counted in roundsRejected rather than silently dropped. entitlement.entitled is false and canSubscribe/canRedeem are both false because USYC is permissioned; the authority is the on-chain canCall() check, which entitlement.howToVerify hands back as a runnable cast command, and entitlement.caveat warns that maxDeposit() returns a non-zero limit even for non-entitled addresses. idleTreasuryUsdc is therefore labeled counterfactual and never touches vault NAV. When Arc cannot be read the route answers 502 rather than ok:true with nulls, or serves the last good read clearly marked stale with its real age." },
