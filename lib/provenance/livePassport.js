// lib/provenance/livePassport.js - a real Strategy Passport (livep-1). ADDITIVE.
// lib/upgrades/strategyPassport.js is NOT edited: its own createStrategyPassport()
// and validatePassport() are used, but fed a real decision.
//
// The defect being fixed: verifyIntegrity() requires traceHash.length === 66,
// while contentHash() returns "sha256:" + 64 hex = 71 characters. That check can
// never become true. Here integrity is verified by RECOMPUTING the hash.
...
// pure: the real integrity check - recompute the hash from the trace record
...
// pure: explains why the built-in check is always false
...
export function makeLivePassport(opts) {
  ...
  return async function livePassport(req, res) {
    ...
    // price cross-check via the existing module (it is fail-open, hence the wrapper)
    let xc = null
    ...
    // decorate() only checks the hash FORMAT and would overwrite the stronger check.
    // Recomputing the hash is strictly stronger, so that is what stays in integrityRecheck.
