...
export function makeLivePassport(opts) {
  ...
  return async function livePassport(req, res) {
    ...
    const validation = Object.assign({}, validatePassport(passport), {
      ...
      integrityNote: "Поле integrity получено устаревшей проверкой длины (ожидает 66 символов) и структурно невыполнимо при формате sha256:<64 hex>. Целостность подтверждается пересчётом хеша в integrityRecheck.",
    })
    const body = {
      ok: true,
