...
export function clampDiagnosis(basePrice) {
  ...
  const minBand = bp // conversionRate >= 0 и explorationBonus >= 0
  ...
  return {
    ...
    explanation: constant
      ? "getPriceBand умножает basePrice на (1 + неотрицательные слагаемые), поэтому "
        + "результат не может быть меньше basePrice, а Math.min(" + PRICE_CAP + ", ...) "
        + "всегда срабатывает. optimalPrice не зависит от наблюдений."
      : "При таком basePrice верхняя граница связывает не всегда.",
  }
}
...
function selfList(env) {
  ...
  const extra = ["0x46213abeca58cc9a89a269fd25a8737c700ca164"] // стейкинг-кошелёк проекта
  ...
  return async function liveThompson(req, res) {
    ...
    if (!payer) {
      return refuse(res, {
        ...
        note: "Лояльная цена определяется историей конкретного плательщика. Без адреса "
          + "считать нечего, а подстановка purchases=0, successRate=0.5 — выдумка.",
      })
    }
    ...
    if (!rec.receipts.length) {
      return refuse(res, { reason: "no_receipts", clampDiagnosis: clamp, note: "Платежей нет: истории лояльности не существует." })
    }
    ...
    if (hist.purchases === 0) {
      return refuse(res, {
        ...
        note: "У адреса нет ни одного оплаченного чека. Возвращается базовая цена, а не "
          + "цена из априорного распределения Beta(1,1), которое ничего не наблюдало.",
      })
    }
    ...
    if (!(sr > 0 && sr <= 1) || !accepted) {
      return refuse(res, {
        ...
        note: "Наблюдений конверсии не существует: журнала показов цены нет, A/B-экспериментов "
          + "не проводилось. Параметры Beta задать не из чего. Значение можно передать "
          + "вручную только с явным признанием, что оно является допущением, а не данными.",
      })
    }
    ...
    const body = {
      ...
      assumptions: [{
        ...
        why: "конверсия по цене не логируется",
      }],
      customerSignal: hist.selfGenerated
        ? "нет: адрес принадлежит проекту, лояльность собственных кошельков не является клиентским сигналом"
        : (hist.externalPayers > 0 ? "внешний плательщик" : "нет внешних плательщиков"),
      upstream: inner,
    }
    return res.status(200).json(decorate(body, {
      synthetic: false,
      source: "lib/upgrades/thompsonSampling.js на реальных чеках " + rec.source,
      endpointKind: "thompson-price",
      computation: clamp.constant ? "posterior_computed_but_discarded_by_cap" : "real_beta_sampling",
      inputs: "покупки из ончейн-чеков (" + rec.count + " платежей), successRate — переданное допущение",
      note: (clamp.constant
        ? "ЦЕНА ПОСТОЯННА: " + PRICE_CAP + " USDC при любых наблюдениях, потому что верхняя "
          + "граница в calculateLoyalPrice всегда связывает. Постериор виден в posterior, но на цену не влияет. "
        : "")
        + "Ответ воспроизводим: генератор засеян хешем входов.",
    }))
  }
}
...
export function calculateUncappedBasePrice(currentCap, targetMultiplier = 1.0) {
  ...
  const safeMargin = 0.999; // 0.1% запаса
