// lib/provenance/liveThompson.js - live pricing (livet-1). ADDITIVE.
// lib/upgrades/thompsonSampling.js is NOT edited: its handler is called with real
// purchases and under a seeded random number generator.
//
// Three defects of the original endpoint, recorded here:
// 1. optimalPrice is MATHEMATICALLY CONSTANT. getPriceBand yields
//    basePrice * (1 + conversionRate*0.5 + explorationBonus), both terms being >= 0,
//    so the result is >= basePrice. calculateLoyalPrice then clamps it with
//    Math.min(0.0009, ...). At basePrice 0.001 the answer is always 0.0009: the
//    posterior is computed and thrown away.
// 2. The answer is IRREPRODUCIBLE: Math.random() in sample() and in explorationBonus.
// 3. purchases/successRate came from the query and defaulted to 0/0.5, meaning the
//    'observations' were never observed.
...
// pure: proves the price is constant for a given basePrice
...
// pure: a deterministic generator seeded from the hash of the inputs (mulberry32)
...
// runs foreign code under a seeded Math.random and RESTORES the global afterwards
...
// pure: the real purchase history of one specific payer
...
export function makeLiveThompson(opts) {
  ...
  return async function liveThompson(req, res) {
    ...
    // Conversion (bought / was shown a price) is logged nowhere: no pricing experiment was ever run.
...
/**
 * 1. ASYNC MUTEX (protection against data races on Vercel)
 * Prevents the global Math.random from being overwritten by concurrent requests.
 */
...
/**
 * A concurrency-safe wrapper around withSeededRandom
 */
...
/**
 * 2. ACTIONABLE PRICE SUGGESTION
 * When clampDiagnosis shows the price is nailed to the cap,
 * this function computes the smallest basePrice at which the cap stops binding.
 */
export function calculateUncappedBasePrice(currentCap, targetMultiplier = 1.0) {
  // If getPriceBand multiplies by (1 + conversionRate*0.5 + explorationBonus),
  // then the minimum multiplier is 1.0 (when conversionRate=0 and bonus=0).
  // For Math.min(cap, basePrice * 1.0) not to bind, basePrice must be strictly < cap.
  const safeMargin = 0.999; // 0.1% headroom
  return Number((currentCap * safeMargin / targetMultiplier).toFixed(6));
}

/**
 * 3. REPUTATION TIER MAPPING (for on-chain reputation)
 * Converts purchase history into a simple "Tier" to be written to CronusReputation.sol,
 * avoiding the need to pass raw arrays into the smart contract.
 */
...
// Extend the export while keeping the old one
...
export const EXTENDED_LIVE_THOMPSON_ROUTES = { 
  ...
  // A new mutex-backed endpoint can be added here if it is ever needed
};
