...
const ADDR = (process.env.CRONUS_DECISIONS_ADDRESS || "0x6190E140F6a643E16D0B2f61FCDA5883430BE310")
...
const arc = defineChain({ id: CHAIN_ID, name: "arc-mainnet", nativeCurrency: { name: "USDC", symbol: "USDC", decimals: 18 }, rpcUrls: { default: { http: [RPC] } } })
...
export default async function decisions(req, res) {
  try {
    ...
    res.status(200).json({ ok: true, network: "arc-mainnet", contract: ADDR, explorer: "https://explorer.arc.io/address/" + ADDR, total_on_chain: count, returned: rows.length, decisions: rows, note: "On-chain decision log (CronusDecisions.logDecision). Oracle is the self-operated demo agent wallet (labeled). Each decision embeds a short sha256 trace hash re-verifiable at /api/trace.", updatedAt: new Date().toISOString() })
  } catch (e) {
