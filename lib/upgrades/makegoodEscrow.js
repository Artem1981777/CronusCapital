...
export default async function handler(req, res) {
  ...
  const raw = await kvCmd(["LRANGE", "cronus:stakes:ledger", "0", "199"])
    // kvCmd returns null when the REST call fails or was never configured. An empty
  // ledger returns [], so the two ARE distinguishable - and collapsing them into []
  // reported "total: 0, positions: []" for an unreachable ledger. That is a
  // fabricated fact, and the contract says missing data must produce a refusal.
  if (!Array.isArray(raw)) {
    ...
    return res.status(200).json({
      ok: false, kind: "makegood",
      reason: configured ? "ledger_unreachable" : "kv_not_configured",
      dataProvenance: { version: "contract-1", synthetic: false, live: false, refusal: true,
        source: "kv:cronus:stakes:ledger",
        note: "The stake ledger could not be read. Reporting no positions instead of zero positions." },
    })
  }
  const list = raw
  const positions = []
  ...
  res.status(200).json({
    ...
    // Labeled here, not by the wrapper: only this handler knows the source key.
    dataProvenance: { version: "contract-1", synthetic: false, live: true, refusal: false,
      source: "kv:cronus:stakes:ledger (LRANGE 0..199)",
      computation: "slashDestination() applied per position; no funds move on GET",
      note: "makegoodPositions counts positions that would actually compensate a buyer. Zero means no such position exists in the live ledger, not that the feature is inert." },
  })
}
