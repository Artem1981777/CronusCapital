...
// Различает ФАБРИКАЦИЮ (число, которого нет в рынке) и ОШИБКУ ПРОГНОЗА (не сбывшееся предсказание).
// ADDITIVE: переиспользует lib/priceSources.js, не меняя в нём ни строки.
// Отличие: priceSources fail-OPEN (agree=null не влияет на вердикт — верно для аннотации).
// FactGuard fail-CLOSED: нет подтверждения 2-го источника => ok:false, расход запрещён.
// fg-2: классификация по ТОКЕНАМ, а не по подстроке ("spotPrice" больше не ловится на "tp").
...
// pure: вытаскивает числовые утверждения о цене из любого вложенного вывода LLM
...
// pure: детерминированный вердикт. Никакой сети, никакого LLM.
...
// сеть изолирована: единственный async-вход, использует ТВОЙ crossCheck
